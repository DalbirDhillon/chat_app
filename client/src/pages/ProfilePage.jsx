...
const ProfilePage = () => {
  ...
  return (
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
        ...
        border-gray-600 flex flex-row gap-4 p-6 max-sm:flex-col-reverse rounded-lg shadow-lg'>
            ...
            className='flex flex-col gap-5 p-10 flex-1'>
                ...
                <label htmlFor="avatar" className='flex items-center gap-3 cursor-pointer'>
                    ...
                    className={`w-12 h-12  ${selectedImg ? 'rounded-full object-contain' : ''}`}/>                                             {/*change to ternary iterator from && */}
                    Upload profile image
                </label>
                <input onChange={(e)=>setName(e.target.value)} value={name}
                type="text" required placeholder='Your name' className='w-full p-2 border border-gray-500 
                rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500'/>

                <textarea onChange={(e)=>setBio(e.target.value)} value={bio}
                name="Write profile bio" required className='p-2 border border-gray-500 
            ...
            <img className={`max-w-44 aspect-square rounded-full mx-10 max-sm:mt-10 self-center 
